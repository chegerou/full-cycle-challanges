...
export default class PlaceOrderUseCase implements UseCaseInterface {
  ...
  private async validateProducts(input: PlaceOrderInputDto): Promise<void> {
    if (input.products.length === 0) {
      return Promise.reject(new Error("No products selected"));
    }

    await this.validateStock(input);
  }

  private async validateStock(input: PlaceOrderInputDto): Promise<void> {
    for (const p of input.products) {
      ...
      if (product.stock <= 0) {
        return Promise.reject(
          new Error(`Product ${p.productId} is not available in stock`)
        );
      }
    }
  }
